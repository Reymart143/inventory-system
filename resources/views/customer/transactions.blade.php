        ...
        <div class="card my-4">
          ...
          <div class="card-body px-0 pb-2">
            ...
            <div class="table-responsive p-3" style="overflow-x: hidden !important;">
                ...
                <table class="table table-bordered table-striped ps-3" id="customer_table">
                    <thead>
                    <tr>
                        ...
                        <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7">Date Ordered</th>
  ...
  <div class="modal-dialog modal-xl">
      <div class="modal-content">
          <div class="modal-header bg-success">
              <h5 class="modal-title text-white" id="addStockProductModalLabel">Transaction Order Product</h5>
            ...
            <form id="stockProductForm" >
              <div class="row">
                <div class="col-md-4 mb-3" id="selectNo">
                  <label for="no_stockout">How many products you bought?</label>
                  <div class="d-flex align-items-center"> 
              ...
              <div class="row d-none" id="Show">
                ...
                <div class="col-md-6">
                    <input type="hidden" id="hidden_stock_${index}" value="0"> 
                      <label for="stock_${index}">Reference No.</label>
                      <input type="text" class="form-control ps-2 modal-inputs ref-input" placeholder="Ref. no" id="reference_no" name="reference_no" required>
                  </div>
                  <div class="col-md-6 mb-3">
                    {{-- <input type="hidden" id="hidden_stock_${index}" value="0">  --}}
                      <label for="stock_${index}">Proof of Receipt.</label>
                      <input type="file" class="form-control ps-2 modal-inputs stockout-input" placeholder="Upload Image" id="image" name="image" required>
                  </div>
          ...
          <div class="modal-footer">
              ...
              <button type="button" id="stockoutproductbtn" class="btn btn-success">Submit Order</button>
          </div>
...
<script>
    $(document).ready(function() {
        var stockoutproductdataTable = $('#customer_table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('customer-transactions') }}", 
            columns: [
                {
                    data: 'product_name',
                    name: 'product_name',
                ...
                //     name: 'image',
                //     render: function(data) {
                //         return '<img src="' + data + '" alt="Image" style="max-width: 50px; max-height: 50px;"/>'
                //     }
                // },
                {
                    data: 'status',
...
<script>
    $(document).ready(function() {
        ...
        function addProductEntry(index) {
            ...
            <div class="row mb-3 product-entry border p-3 rounded" id="product_entry_${index}">
                ...
                <input type="hidden" id="type_${index}" name="type[]" value="1">
                <div class="col-md-3">
                ...
                <div class="col-md-4 mt-4">
                    <input type="hidden" id="weeks_${index}" name="weeks[]">
                        <input type="hidden" id="reference_no_${index}" name="reference_no[]">
                            <input type="hidden" id="image_${index}" name="image[]">
                                <input type="hidden" id="image_${index}" name="image-tag[]">
        ...
        $('#reference_no').on('change', function() {
            ...
            // Update the hidden input for all existing product entries with the selected week
            $('.product-entry').each(function(index) {
                $(this).find('input[id^=reference_no_]').val(RefNo); // Set the hidden week's input value
            });
        });
        ...
        if (filesSelected) {
            ...
            reader.onload = function () {
                // Log the base64 image content for debugging (optional)
                console.log(reader.result);

                // Update the value of the hidden input field (if needed)
    ...
    $('#stockoutproductbtn').on('click', function (e) {
            ...
            $('.product-entry').each(function() {
                ...
                const RefNo = $(this).find('input[id^=reference_no_]').val();
                const stockInput = $(this).find('.stockout-input');
                const Image = $(this).find('input[id^=image_]');
